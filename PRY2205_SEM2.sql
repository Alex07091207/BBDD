...
SELECT
  numfactura,
  TO_CHAR(fecha, 'DD Month YYYY') AS "Fecha de emisión",
  LPAD(rutcliente, 10, '0') AS rutcliente,
 TO_CHAR(neto, '$999G999G990') AS "Monto Neto",
  TO_CHAR(iva, '$999G999G990') AS "Monto Iva",
  TO_CHAR(total, '$999G999G990') AS "Total Factura",
  (CASE
  WHEN total BETWEEN 0 AND 50000 THEN 'Bajo'
  WHEN total BETWEEN 50001 AND 100000 THEN 'Medio'
  WHEN total >100000 THEN 'Alto'
  END) AS "Categoría de monto",
  (CASE
...
--CASO 2--
SELECT 
  LPAD(LTRIM(rutcliente), 12,'*') AS "RUT",
  nombre AS "Cliente",
  NVL(TO_CHAR(telefono), 'Sin teléfono') AS "TELEFONO", 
  NVL(TO_CHAR(codcomuna), 'Sin comuna') AS "COMUNA",
  estado AS "ESTADO",
  (CASE
    WHEN (saldo / credito) < 0.50 THEN 
      'Bueno ( ' || TO_CHAR(credito - saldo, '$999G999G990') || ')'
    WHEN (saldo / credito) BETWEEN 0.50 AND 0.80 THEN 
      'Regular ( ' || TO_CHAR(saldo, '$999G999G990') || ')'
    WHEN (saldo / credito) > 0.80 THEN 
      'Crítico'
  END) AS "Estado Crédito",
  (CASE
    WHEN INSTR(mail, '@') > 0 THEN 
      UPPER(SUBSTR(mail, INSTR(mail, '@') + 1))
    ELSE 
...
SELECT 
  ...
  (CASE
    ...
    ELSE TO_CHAR(VALORCOMPRADOLAR) || ' USD'
  END) AS "Compra en USD",
  (CASE
    WHEN VALORCOMPRADOLAR IS NULL THEN 'Sin registro'
    ELSE TO_CHAR(VALORCOMPRADOLAR * 950, '$999G999G990') || ' PESOS'
  ...
  TOTALSTOCK AS "Stock",
    ...
    WHEN TOTALSTOCK > &umbral_alto THEN 'OK'
  END) AS "Alerta Stock",
  (CASE
    WHEN TOTALSTOCK > 80 THEN 
      TO_CHAR(VUNITARIO * 0.10, '$99G999G999G990')
    ELSE 
      'N/A'
